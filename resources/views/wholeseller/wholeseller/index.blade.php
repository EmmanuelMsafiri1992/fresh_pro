...
<form action="{{ route('superadmins.wholesellers.index') }}" method="GET" role="search">
...
<a class="btn btn-secondary" href="{{ route('superadmins.wholesellers.pdf') }}">
<i class="fas fa-download"></i> @lang('Export')
</a>
<a href="{{ route('superadmins.wholesellers.create') }}" class="btn btn-primary">
...
<td>
<a href="{{ route('superadmins.wholesellers.edit', $wholeseller->id) }}"> {{ $wholeseller->name }}</a>
</td>
...
@if ($wholeseller->isActive())
<a href="{{ route('superadmins.wholesellers.status', $wholeseller->id) }}"
...
{{ __('Active') }}</a>
@endif
<a href="{{ route('superadmins.wholesellers.edit', $wholeseller->id) }}"
class="dropdown-item"><i class="fas fa-edit"></i>
{{ __('Edit') }}</a>

@if(Auth::guard('superadmin')->check()) <a href="{{ route('superadmins.wholesellers.delete', $wholeseller->id) }}"
...
@if($message=Session::get('wholeseller-saved'))
<script>
Swal.fire({
  title: 'Saved!',
  text: '{{ $message }}',
...
@endif
@if($message=Session::get('wholeseller-updated'))
<script>
Swal.fire({
  title: 'Updated!',
  text: '{{ $message }}',
...
@endif
@if($message=Session::get('wholeseller-deleted'))
<script>
Swal.fire({
  title: 'Deleted!',
  text: '{{ $message }}',
...
@endif
@if($message=Session::get('wholeseller-status-updated'))
<script>
Swal.fire({
