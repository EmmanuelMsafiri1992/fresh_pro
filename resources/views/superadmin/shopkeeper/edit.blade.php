...
<div class="col-sm-6">
<h1 class="m-0 text-dark">{{ __('Edit Shopkeeper') }}</h1>
</div>
<div class="col-sm-6">
...
<li class="breadcrumb-item">
<a href="{{ route('superadmins.shopkeepers.index') }}">{{ __('Shopkeepers') }}</a>
</li>
<li class="breadcrumb-item active">{{ __('Edit Shopkeeper') }}</li>
</ol>
</div>
</div>
</div>
...
<h3 class="card-title">{{ __('Edit Shopkeeper') }}: {{ $shopkeeper->name }}</h3>
<div class="card-tools">
<a href="{{ route('superadmins.shopkeepers.index') }}" class="btn btn-block btn-primary">
...
<form class="form-horizontal" action="{{ route('superadmins.shopkeepers.update', $shopkeeper->id) }}" method="post" enctype="multipart/form-data">
@csrf
@method('PUT')
<div class="card-body">
<div class="row">
<div class="col-md-6 form-group">
<label for="name" class="col-form-label">{{ __('Shopkeeper Name') }}<span class="required-field">*</span></label>
<input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" placeholder="{{ __('shopkeeper Name') }}" value="{{ $shopkeeper->name }}" required>
@error('name')
...
<input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" placeholder="{{ __('Email Address') }}" value="{{ $shopkeeper->email }}" readonly>
@error('email')
<span class="invalid-feedback" role="alert">
<strong>{{ $message }}</strong>
</span>
...
<option value="1" {{ $shopkeeper->status==1 ? 'selected' : '' }}>{{ __('Active') }}</option>
<option value="0" {{ $shopkeeper->status==0 ? 'selected' : '' }}>{{ __('Inactive') }}</option>
</select>
@error('status')
<span class="invalid-feedback" role="alert">
<strong>{{ $message }}</strong>
