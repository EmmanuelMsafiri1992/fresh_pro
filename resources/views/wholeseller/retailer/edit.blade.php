...
<div class="col-sm-6">
<h1 class="m-0 text-dark">{{ __('Edit Retailer') }}</h1>
</div>
<div class="col-sm-6">
...
<li class="breadcrumb-item active">{{ __('Edit Retailer') }}</li>
</ol>
</div>
</div>
</div>
...
<h3 class="card-title">{{ __('Edit Retailer') }}: {{ $retailer->name }}</h3>
...
<form class="form-horizontal" action="{{ route('superadmins.retailers.update', $retailer->id) }}" method="post" enctype="multipart/form-data">
@csrf
@method('PUT')
<div class="card-body">
<div class="row">
...
<input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" placeholder="{{ __('retailer Name') }}" value="{{ $retailer->name }}" required>
@error('name')
...
<input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" placeholder="{{ __('Email Address') }}" value="{{ $retailer->email }}" readonly>
@error('email')
<span class="invalid-feedback" role="alert">
<strong>{{ $message }}</strong>
</span>
...
<option value="1" {{ $retailer->status==1 ? 'selected' : '' }}>{{ __('Active') }}</option>
<option value="0" {{ $retailer->status==0 ? 'selected' : '' }}>{{ __('Inactive') }}</option>
</select>
@error('status')
<span class="invalid-feedback" role="alert">
<strong>{{ $message }}</strong>
