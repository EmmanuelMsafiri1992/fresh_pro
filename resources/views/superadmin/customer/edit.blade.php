...
<li class="breadcrumb-item">
<a href="{{ route('superadmins.customers.index') }}">{{ __('Customers') }}</a>
</li>
...
<h3 class="card-title">{{ __('Edit Customer') }}: {{ $customer->name }}</h3>
<div class="card-tools">
<a href="{{ route('superadmins.customers.index') }}" class="btn btn-block btn-primary">
...
<form class="form-horizontal" action="{{ route('superadmins.customers.update', $customer->id) }}" method="post" enctype="multipart/form-data">
@csrf
@method('PUT')
<div class="card-body">
<div class="row">
<div class="col-md-6 form-group">
<label for="name" class="col-form-label">{{ __('Customer Name') }}<span class="required-field">*</span></label>
<input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" placeholder="{{ __('customer Name') }}" value="{{ $customer->name }}" required>
@error('name')
...
<input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" placeholder="{{ __('Email Address') }}" value="{{ $customer->email }}" readonly>
@error('email')
<span class="invalid-feedback" role="alert">
<strong>{{ $message }}</strong>
</span>
