...
<div class="col-sm-6">
<h1 class="m-0 text-dark">{{ __('Edit Wholeseller') }}</h1>
</div>
<div class="col-sm-6">
...
<li class="breadcrumb-item">
<a href="{{ route('superadmins.wholesellers.index') }}">{{ __('Wholesellers') }}</a>
</li>
<li class="breadcrumb-item active">{{ __('Edit wholeseller') }}</li>
</ol>
</div>
</div>
</div>
...
<h3 class="card-title">{{ __('Edit Wholeseller') }}: {{ $wholeseller->name }}</h3>
<div class="card-tools">
<a href="{{ route('superadmins.wholesellers.index') }}" class="btn btn-block btn-primary">
...
<form class="form-horizontal" action="{{ route('superadmins.wholesellers.update', $wholeseller->id) }}" method="post" enctype="multipart/form-data">
@csrf
@method('PUT')
<div class="card-body">
<div class="row">
<div class="col-md-6 form-group">
<label for="name" class="col-form-label">{{ __('Wholeseller Name') }}<span class="required-field">*</span></label>
<input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" placeholder="{{ __('wholeseller Name') }}" value="{{ $wholeseller->name }}" required>
@error('name')
...
<input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" placeholder="{{ __('Email Address') }}" value="{{ $wholeseller->email }}" readonly>
@error('email')
<span class="invalid-feedback" role="alert">
<strong>{{ $message }}</strong>
</span>
